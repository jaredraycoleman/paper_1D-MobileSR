...
\begin{equation}
\label{eq:cr-case1}
CR_1 = \frac{2 \cdot \frac{d+s}{1-v} - s}{\frac{2(d-s)}{1-v} + s} = -1 - \frac{4d}{-2d + s + sv}.
\end{equation}
...
\begin{equation}
\label{eq:cr-case2}
CR_2 = \frac{(v-1)\left(-s + \frac{2\left(2s + \frac{d-s}{u^2 - uv}\right)}{1+u}\right)}{-2d + s + sv}.
\end{equation}
...
If the speed $v$ is known, the fast agent could alternatively execute the single-agent algorithm from Section~\ref{Speed $v$ is Known to the Searcher}, which achieves a competitive ratio of
$$
CR_{solo} = 1 + \frac{v + \sqrt{2-v^2}}{1-v}.
$$
...
Determining the exact conditions under which $CR_2 \leq CR_{solo}$ leads to a complicated expression in $u$, $v$, $s$, and $d$. The optimal strategy selection depends on the specific values of these parameters.
