...
One option is for Hopps to simply execute the single-agent algorithm by herself, ignoring Clawhauser entirely. However, we consider an alternative approach which we call the \emph{split-up algorithm}. In this algorithm, Hopps moves toward the jail (origin) while Clawhauser moves away from the jail. If Hopps reaches the jail without finding the fugitive, then it must be that $d > s$, and so she starts moving away from the jail.
